...
@php
    $entityId = isset($data->id) ? $data->id : 0;
...
<section class="content-header">
    <h1>
        @if(empty($data))
        Add / {{trans('labels.entity')}}
        @else
        &nbsp;
            Edit / {{$data->name}} / {{$data->entityType->name ?? ''}}
            <a class="pull-left" href="{{ url('/admin/allentity') }}" title="Back">
...
<section class="content">
        ...
        <div class="col-md-12">
            <div class="box">
                <div class="box-header with-border">
                    ...
                    @if(!empty($data))
                    <a class="pull-right" href="{{ getUrlOfEntityDetail($data) }}" target="_blank" title="Permalink">
                        Permalink
                    </a>
                    @endif
                </div><!-- /.box-header -->
                <form id="addbusiness" class="form-horizontal" method="post" action="{{ url('admin/user/business/savebusinessinfo') }}" enctype="multipart/form-data">
                    ...
                    <div class="box-body">
                        <div class="form-group">
                            <?php
                            if (old('is_normal_view'))
                                $is_normal_view = old('is_normal_view');
                            elseif (isset($data) && !empty($data))
                                $is_normal_view = $data->is_normal_view;
                            else
                                $is_normal_view = 1;
                            ?>
                            <label for="title" class="col-sm-2 control-label">Front-End View</label>
                            <div class="col-sm-2">
                                <input type="radio" class="check_box_set" name="is_normal_view" value="1" @if($is_normal_view) checked @endif /> Normal View
                            </div>
                            <div class="col-sm-3">
                                <input type="radio" class="check_box_set"  name="is_normal_view" value="0" @if(!$is_normal_view) checked @endif /> Extented View
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            if (old('asset_type_id'))
                                $entity_type = old('asset_type_id');
                            elseif (isset($data) && !empty($data))
                                $entity_type = $data->asset_type_id;
                            else
                                $entity_type = '';
                            ?>
                            <label for="entity_type" class="col-sm-2 control-label">{{trans('labels.entity_type')}}<span class="star_red">*</span></label>
                            <div class="col-sm-8">
                                
                                <select name="asset_type_id" class="form-control" style="cursor: pointer;" id="entity_type" >
                                    <option value="">Select {{trans('labels.entity')}}</option>
                                    @forelse($allEntities as $entity)
                                        <option value="{{$entity->id}}" @if($entity_type == $entity->id) selected @endif>
                                            {{$entity->name}}
                                        </option>
                                    @empty
                                    @endforelse
                        ...
                        <div class="form-group" id="businessUserSection" style="display: none;">
                            <?php
                            if (old('business_user'))
                                $business_user = old('business_user');
                            elseif (isset($data) && !empty($data))
                                $business_user = $data->user_id;
                            else
                                $business_user = '';
                            ?>
                            ...
                            <div class="col-sm-8">
                                ...
                                <select name="business_user" class="form-control" style="cursor: pointer;" id="business_user" >
                                    <option value="">Select {{trans('labels.user')}}</option>
                                    @if(!empty($userDetail))
                                        <option value="{{$userDetail->id}}" selected>{{$userDetail->name}}</option>
                                    @endif
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            elseif (isset($data) && !empty($data))
                                ...
                                $short_description = '';
                            ?>
                            <label for="short_description" class="col-sm-2 control-label">{{trans('labels.short_description')}}<span class="star_red">*</span></label>
                            <div class="col-sm-8">
                               <textarea type="text" class="form-control description" id="short_description" name="short_description" placeholder="{{trans('labels.short_description')}}">{{$short_description}}</textarea>
                            </div>
                            <div class="col-sm-2">
                                <button type="button" id="addBtnLangOfShort" class="btn bg-purple">Add in Other Language</button>
                                @if(isset($data->otherLangDescriptions))
                                @foreach($data->otherLangDescriptions as $description)
                                    <input type="hidden" name="short_descriptions[{{$description->language}}]" value="{{$description->short_description}}">
                                @endforeach
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-8">
                               <textarea type="text" class="form-control description" id="description" name="description" placeholder="{{trans('labels.description')}}">{{$description}}</textarea>
                            </div>
                            <div class="col-sm-2">
                                <button type="button" id="addBtnLang" class="btn bg-purple">Add in Other Language</button>
                                @if(isset($data->otherLangDescriptions))
                                @foreach($data->otherLangDescriptions as $description)
                                    <input type="hidden" name="descriptions[{{$description->language}}]" value="{{$description->description}}">
                                @endforeach
                                @endif
                            </div>                            
                        </div>
                        ...
                        <div class="form-group">
                            <?php
                            if (old('seo_meta_tags'))
                                $seo_meta_tags = old('seo_meta_tags');
                            elseif (isset($data) && !empty($data))
                                $seo_meta_tags = $data->seo_meta_tags;
                            else
                                $seo_meta_tags = '';
                            ?>
                            <label for="seo_meta_tags" class="col-sm-2 control-label">SEO Meta tags</label>
                            <div class="col-sm-8">
                                <input type="text" class="form-control" id="seo_meta_tags" name="seo_meta_tags" value="{{$seo_meta_tags}}">
                            </div>
                        </div>
                        <div class="form-group">
                            <?php
                            if (old('seo_meta_description'))
                                $seo_meta_description = old('seo_meta_description');
                            elseif (isset($data) && !empty($data))
                                $seo_meta_description = $data->seo_meta_description;
                            else
                                $seo_meta_description = '';
                            ?>
                            <label for="seo_meta_description" class="col-sm-2 control-label">SEO Meta description</label>
                            <div class="col-sm-8">
                                <textarea  class="form-control" id="seo_meta_description" name="seo_meta_description" placeholder="Please enter SEO meta description">{{$seo_meta_description}}</textarea>
                            </div>
                        </div>
                    </div><!-- /.box-body -->
            ...
            @if($entityId > 0)
            <div class="box section_video">
                <div class="box-header with-border">
                    <h3 class="box-title">Video sharing</h3>
                </div><!-- /.box-header -->
                <form id="videoForm" class="form-horizontal" method="post" action="{{ route('save.entity.video') }}" enctype="multipart/form-data">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" name="entity_id" value="{{$entityId}}">
                    
                    <div class="box-body" id="videoBody">
                        @include('Admin.Entity.video-sharing',['videos' => $data->videos])
                    </div><!-- /.box-body -->
                    <div class="box-footer">
                        <div class="pull-right">
                            <button type="button" id="add-video-more-btn" class="btn  save-btn" style="margin-right: 10px;">Add More</button>
                            <button type="submit" class="btn bg-purple save-btn">{{trans('labels.savebtn')}}</button>
                        </div>
                    </div><!-- /.box-footer -->
                </form>
            </div>
            <div class="box section_know_more">
                <div class="box-header with-border">
                    <h3 class="box-title">Know more</h3>
                </div><!-- /.box-header -->
                <form id="knowMoreForm" class="form-horizontal" method="post" action="{{ route('save.know.more') }}" enctype="multipart/form-data">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" name="entity_id" value="{{$entityId}}">
                    
                    <div class="box-body" id="knowMoreBody">
                        @include('Admin.Entity.know-more',['knowMores' => $data->knowMores,'selectedLanguage' => 'english'])
                    </div><!-- /.box-body -->
                    <div class="box-footer">
                        <div class="pull-right">
                            <button type="button" id="add-know-more-btn" class="btn  save-btn" style="margin-right: 10px;">Add More</button>
                            <button type="submit" class="btn bg-purple save-btn">{{trans('labels.savebtn')}}</button>
                        </div>
                    </div><!-- /.box-footer -->
                </form>
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <!-- Horizontal Form -->
            <div class="box section_contact_details">
                <div class="box-header with-border">
                    <h3 class="box-title">
                        Contact Details
                    </h3>
            ...
            <!-- Horizontal Form -->
            <div class="box section_hours_of_opration">
                <div class="box-header with-border">
            ...
            <div class="box section_business_activities">
                <div class="box-header with-border">
            ...
            @if($businessDoc && !$businessDoc->isEmpty())
            <div class="box section_business_documnet">
                <div class="box-header with-border">
        ...
        <div class="col-md-6">
            <!-- Horizontal Form -->
            <div class="box section_category">
                <div class="box-header with-border">
                    <h3 class="box-title">
                   ...
                   <?php
                    ...
                    <div id="edit_cat" style="display:inline;">
                        <div class="form-group">
                            ...
                            $categorySelectedId = '';
                            if(isset($data->category_hierarchy) && $data->category_hierarchy)
                            {
                                $categorySelectedId = substr($data->category_hierarchy, 0, 1); 
                            }
                            ?>
                            <label for="category_id" class="col-sm-2 control-label">{{(isset($data) && !empty($data)) ? trans('labels.category') : trans('labels.category') }}</label>
                            <div class="col-sm-8">
                                <select name="parent_category" id="parent_category" data="" class="form-control" onchange="return getParentSubCategory(this.value,1);">
                        ...
                        <div class="form-group">
                            ...
                            <div id="selected_subcat" class="col-sm-8">
                                <?php 
                                    $categoryArray = isset($data->category_id) ? $data->category_id : '' ; 
            ...
            <!-- Horizontal Form -->
            <div class="box section_near_by">
                <div class="box-header with-border">
                    <h3 class="box-title">
                        Near By
                    </h3>
                </div><!-- /.box-header -->
                <form id="nearByForm" class="form-horizontal" method="post" action="{{ route('save.nearBy.filter') }}" enctype="multipart/form-data">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" name="entity_id" value="{{$entityId}}">
                    <div class="box-body" id="nearByBody">
                        @forelse($data->nearByFilter as $nearByFilter)
                            @include('Admin.Entity.near-by',['nearByFilter' => $nearByFilter,'deleteBtn' => ($loop->first) ? false : true ])
                        @empty
                        @include('Admin.Entity.near-by',['nearByFilter' => null, 'deleteBtn' => false])
                        @endforelse          
                    </div><!-- /.box-body -->
                    <div class="box-footer">
                        <div class="pull-right">
                            <button type="button" id="add-near-by-more-btn" class="btn  save-btn" style="margin-right: 10px;">Add More</button>
                            <button type="submit" class="btn bg-purple save-btn">{{trans('labels.savebtn')}}</button>
                        </div>
                    </div><!-- /.box-footer -->
                </form>
            ...
            <!-- Horizontal Form -->
            <div class="box section_social_profiles">
                <div class="box-header with-border">
                    <h3 class="box-title">
                        Social Profiles
                    </h3>
            ...
            <div class="box section_online_stores">
                <div class="box-header with-border">
                    <h3 class="box-title">
                        Online Stores
                    </h3>
            ...
            <div class="box section_public_website">
                <div class="box-header with-border">
    ...
    <div class="row">
        <!-- right column -->
        <div class="col-md-12">
            <div class="box section_custom_component">
                <div class="box-header with-border">
                    <h3 class="box-title">Custom Details</h3>
                </div><!-- /.box-header -->
                <form id="customDetailForm" class="form-horizontal" method="post" action="{{ route('save.custom.detail') }}" enctype="multipart/form-data">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" name="entity_id" value="{{$entityId}}">
                    
                    <div class="box-body" id="customDetailBody">
                        @include('Admin.Entity.know-more',['knowMores' => $data->customDetails,'selectedLanguage' => 'english', 'langId' => 'customDetailLang','descClass' => 'custom-detail-desc'])
                    </div><!-- /.box-body -->
                    <div class="box-footer">
                        <div class="pull-right">
                            <button type="button" id="add-custom-detail-btn" class="btn  save-btn" style="margin-right: 10px;">Add More</button>
                            <button type="submit" class="btn bg-purple save-btn">{{trans('labels.savebtn')}}</button>
                        </div>
                    </div><!-- /.box-footer -->
                </form>
        ...
        <div class='modal-dialog modal-dialog-centered'>
            <div class='modal-content'>
                <div class='modal-header'>
                    <h3>Add description in multiple languages<button type='button' class='close pull-right' data-dismiss='modal'>&times;</button></h3>                    
                </div>
                <div class='modal-body'>
                    <div class="row">
                        <div class="col-md-12">
                        <div class="form-group">  
                            <label for="title" class="control-label">{{trans('labels.language')}}</label>
                            <div class="">
                                <select name="language" id="descLang" class="form-control" >
                                    @foreach($othersLangs as $key => $language)
                                        <option value="{{$language}}" {{($key == 0)?'selected':''}}>{{$language}} </option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group">                            
                            <label for="description" class="control-label">{{trans('labels.description')}}</label>
                            <div class="">
                                <textarea type="text" id="descInOtherLang" class="form-control description"  name="descInOtherLang" placeholder="{{trans('labels.description')}}"></textarea>
                            </div>
                        </div>
                        </div>
                    </div>
                    <input type="hidden" id="descType" value="short"/>
                </div>
                <div class='modal-footer'>
                    <button type='button' id="saveLangDescBtn"  class="btn btn-success"> Add</button>
...
<script type="text/javascript">
    ...
    $('#seo_meta_tags').tagEditor({
        placeholder: 'Enter SEO meta tags ...',
    });

	$(function() {
        initializeSelect2();       
        $('#business_user').select2({
        ...
        $("#entity_type").change(function() {
            if($("#entityId").val() > 0) {
                ...
                $("#business_user").val('').trigger('change.select2');
            }
            var entityType = $.trim($(this).find("option:selected").text());
            if(entityType.toLowerCase() == 'business') {
                $("#businessUserSection").show();
            } else {
                $("#businessUserSection").hide();
            }
            getEnableComponent();
        });
        ...
        $(document).on("change","#knowMoreLang",function() {
			var language =  $(this).val();
            // if(language != 'english') {
            //     $("#add-know-more-btn").prop('disabled',true);
            // } else {
            //     $("#add-know-more-btn").prop('disabled',false);
            // }
            $.ajax({
                type: "POST",
                url: "{{route('getEntityKnowMore')}}",
                data: {from:'admin',entity_id: $("#entityId").val(), language: language},
                success: function( response ) {                    
                    if(response.status) {
                        $("#knowMoreBody").html(response.data);
                        CKEDITOR.replaceAll( 'know-more-description' );                        
                    }
                }
            });
		});

        $(document).on("change","#customDetailLang",function() {
			var language =  $(this).val();
            $.ajax({
                type: "POST",
                url: "{{route('getEntityCustomDetails')}}",
                data: {from:'admin',entity_id: $("#entityId").val(), language: language},
                success: function( response ) {                    
                    if(response.status) {
                        $("#customDetailBody").html(response.data);
                        CKEDITOR.replaceAll( 'custom-detail-desc' );                        
                    }
                }
            });
		});

        // open modal for add description in multi language
        $("#addBtnLang").click(function() {
            $("#descType").val('desc');
            setDescriptions();
            $("#descModal").modal('show');
        });

        $("#addBtnLangOfShort").click(function() {
            $("#descType").val('short');
            setDescriptions();
            $("#descModal").modal('show');
        });

        // set description in multi language after change language
        $("#descLang").change(function() {
            setDescriptions();
        });
        
        // save decription in multiple hidden input
        $("#saveLangDescBtn").click(function() {
            var language = $("#descLang").val();
            var descType = $("#descType").val();
            var description = CKEDITOR.instances['descInOtherLang'].getData();
            if(descType == 'short') {
                var input = $("#addBtnLangOfShort").parent().find('input[name="short_descriptions['+language+']"]');
            } else {
                var input = $("#addBtnLang").parent().find('input[name="descriptions['+language+']"]');
            }
            if(input.length > 0) {
                input.val(description);
            } else {
                if(descType == 'short') {
                    $("#addBtnLangOfShort").parent().append("<input type='hidden' name='short_descriptions["+language+"]' value='"+description+"'>");
                } else {
                    $("#addBtnLang").parent().append("<input type='hidden' name='descriptions["+language+"]' value='"+description+"'>");
                }
            }
        ...
        // for video fields
        $("#add-video-more-btn").click(function() {
            let title = $(".video-title").first().clone();
            let tumbnail = $(".video-thumbnail").first().clone();
            let placeHolder = "{{trans('labels.description')}}";
            let descId = "desc"+Math.floor(1000 + Math.random() * 9000);
            let description = '<div class="form-group"><label for="description" class="col-sm-2 control-label">'+placeHolder+'</label><div class="col-sm-8"><textarea id="'+descId+'" type="text" class="form-control"  name="description[]" placeholder="'+placeHolder+'"></textarea></div></div>';
            let input = '<div class="extra-video"><input type="hidden" class="form-control video-id"  name="id[]"><div class="form-group">'+title.html()+'<div class="col-sm-1" style="padding-left: 0px;"> <span style="cursor: pointer;" class="badge bg-red remove-video" ><i class="fa fa-remove"></i></span></div></div>'+description+'<div class="form-group">'+tumbnail.html()+'</div> <div class="form-group"><label for="video" class="col-sm-2 control-label">Video</label><div class="col-sm-4"><input type="file" name="video[]"></div></div></div>';
            $(this).closest('form').find('.box-body').append(input);
            CKEDITOR.replace(descId);
        });

        // for delete video
        $(document).on("click",".remove-video",function() {
            var deletedId = $(this).closest('.extra-video').find('.video-id').val();
            console.log('deleted Video:- '+deletedId);
            if( deletedId > 0) {
                $("#videoForm").append('<input type="hidden" name="deletedVideos[]" value="'+deletedId+'">');
            }
            $(this).closest('.extra-video').remove();
        });

        // for know more fields
        $("#add-know-more-btn,#add-custom-detail-btn").click(function() {
            let title = $(".know-more-title").first().clone();
            let placeHolder = "{{trans('labels.description')}}";
            let descId = "desc"+Math.floor(1000 + Math.random() * 9000);
            let description = '<div class="form-group"><label for="description" class="col-sm-2 control-label">'+placeHolder+'</label><div class="col-sm-8"><textarea id="'+descId+'" type="text" class="form-control"  name="description[]" placeholder="'+placeHolder+'"></textarea></div></div>';
            let knowMoreInput = '<div class="extra-know-more"><div class="form-group">'+title.html()+'<div class="col-sm-1" style="padding-left: 0px;"> <span style="cursor: pointer;" class="badge bg-red remove-know-more" ><i class="fa fa-remove"></i></span></div></div>'+description+'</div>';
            $(this).closest('form').find('.box-body').append(knowMoreInput);
            CKEDITOR.replace(descId);
        });

        // for delete know more
        $(document).on("click",".remove-know-more",function() {
            $(this).closest('.extra-know-more').remove();
        });

        // for add near by multiple
        $("#add-near-by-more-btn").click(function() {
            $('.near-by-select2').select2("destroy");
            let title = $(".near-by").last().clone().appendTo('#nearByBody');
            // $('.near-by-switch').each(function( index ) {
            //     $(this).attr('name','is_enable_filter['+index+']');
            // });
            $('.near-by-select2').each(function( index ) {
                $(this).attr('name','asset_type_id['+index+'][]');
            });            
            initializeSelect2();
        });

        // for delete near by
        $(document).on("click",".remove-near-by",function() {
            $(this).closest('.near-by').remove();
        });

        // for custom detail fields
        // $("#add-custom-detail-btn").click(function() {
        //     let title = $(".custom-detail-title").first().clone();
        //     let placeHolder = "{{trans('labels.description')}}";
        //     let descId = "desc"+Math.floor(1000 + Math.random() * 9000);
        //     let description = '<div class="form-group"><label for="description" class="col-sm-2 control-label">'+placeHolder+'</label><div class="col-sm-8"><textarea id="'+descId+'" type="text" class="form-control"  name="description[]" placeholder="'+placeHolder+'"></textarea></div></div>';
        //     let knowMoreInput = '<div class="extra-custom-detail"><div class="form-group">'+title.html()+'<div class="col-sm-1" style="padding-left: 0px;"> <span style="cursor: pointer;" class="badge bg-red remove-custom-detail" ><i class="fa fa-remove"></i></span></div></div>'+description+'</div>';
        //     $("#customDetailBody").append(knowMoreInput);
        //     CKEDITOR.replace(descId);
        // });

        // for delete custom detail
        // $(document).on("click",".remove-custom-detail",function() {
        //     $(this).closest('.extra-custom-detail').remove();
        // });
    ...
    function initializeSelect2() {
        $('.near-by-select2').select2({
            placeholder: 'All entities'
        });
    }

    // set description in multi language
    function setDescriptions() {
        var language = $("#descLang").val();
        var descType = $("#descType").val();
        if(descType == 'short') {
            var description = $("#addBtnLangOfShort").parent().find('input[name="short_descriptions['+language+']"]').val();
        } else {
            var description = $("#addBtnLang").parent().find('input[name="descriptions['+language+']"]').val();
        }
        CKEDITOR.instances['descInOtherLang'].setData(description);
    }

    function getEnableComponent() {
        $(".box:not(:first)").hide();
        var asset_type_id = $("#entity_type").val();
        var category_id = $("input[name='category_id[]']").val();
        if(asset_type_id > 0) {
            $.ajax({
                url: '{{ route("getSubAssetTypeFields") }}',
                method:'POST',
                data: {asset_type_id : asset_type_id, sub_category_id: category_id},
                success: function( response ) {
...
$(document).ready(function () {
    ...
    CKEDITOR.replaceAll( 'know-more-description' );
    CKEDITOR.replaceAll( 'custom-detail-desc' );
    CKEDITOR.replaceAll( 'video-description' );
    ...
    var businessRules = 
    {
        ...
        // business_user: {
        //     required: function(){
        //         var entityType = $.trim($("#entity_type").find("option:selected").text());
        //         return entityType.toLowerCase() == 'business';
        //     }
        // },
        name: {
            required: true
        ...
        {
            required: function(){
                return ($("#entityId").val() == 0 || ($("#entityId").val() > 0 && $('.business_img').length == 0));
            },
    ...
    $("#knowMoreForm").validate({
        rules: {
            ...
            //         CKEDITOR.instances[textarea.name].updateElement(); // update textarea
...
{
    var selectedImages = $("#business_images").val();
    if($('.business_img').length == 1 && selectedImages == '') {
